...
export default class CzFolderStructure extends Vue {
  ...
  protected isFileTooBig(file: IFile) {
    ...
    return (
      file.file?.size &&
      file.file?.size > this.repoMetadata.maxUploadSizePerFile
    );
  }
  ...
  protected async onRenamed(item: IFile | IFolder, name: string) {
    if (name.trim()) {
      ...
      if (this.isEditMode) {
        ...
        const wasRenamed = await this.activeRepository.renameFileOrFolder(
          this.identifier,
          item,
          newPath
        );
  ...
  private async deleteFileOrFolder(item: IFile | IFolder): Promise<boolean> {
    ...
    if (!this.isFolder(item) && this.isFileInvalid(item as IFile)) {
      ...
      wasDeleted = await this.activeRepository.deleteFileOrFolder(
        this.identifier,
        item
      );
    }
  ...
  private async _paste(item, targetFolder) {
    ...
    item.isDisabled = true;
    const wasMoved = await this.activeRepository.renameFileOrFolder(
      this.identifier,
      item,
      newPath
    );
